...
предназначенную для описания какой-либо логики взаимодействия юзера с системой в виде графа состояний.
...
```
Теоретически, "вводом пользователя" и триггером для handler-а может быть почти любой объект, но пока что сфокусируемся на тексте.
...
5. Если коммнда найдена, доступна в текущем состоянии, и существует handler для нее, то вызывается handler c аргументами `user_input, context, *args, **kwargs`, пользователь переходит в состояние привязанное к данному handler в данном состоянии (т.е. происходит переход по графу состояний), и возвращается результат работы handler

По умолчанию доступны следующие базовые классы состояний:
- `State`, самое простое состояние, принцип его работы описан далее
