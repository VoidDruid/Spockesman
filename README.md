...
(или написать свой бэкенд для хранения, для примера можно взять `spockesman.context.backend.redis_backend`), и описать его конфигурацию в питоновском модуле (примеры config.py есть в tests и в example).
...
При использовании redis параметров для настройки больше, их можно найти в tests/config.py
